...
Reads the deal board rather than recomputing anything: ``DealScoreCache`` is
already the answer to "what is underpriced right now", and a notifier that
scored ads itself would be a second implementation of the number the site
shows — the exact drift that produced 123% discounts last time round.

Two rules keep it quiet enough to stay switched on:

* a listing is announced **once, ever** (``NotifiedAd``), so a car that keeps
  qualifying is one message rather than one per tick;
* nothing is sent below the configured discount *and* peer count, because a
  median over a handful of cars is not evidence of a bargain.

Telegram is reached with ``requests``, which is already a dependency for the
crawler. No bot library, no queue, no retry daemon: a missed message is
recoverable by the deal board still showing the car.
"""
...
# (a fresh install, a threshold lowered); dumping hundreds of messages into a
# chat is worse than truncating and picking the rest up next tick.
MAX_PER_RUN = 10


def _candidates(cfg: NotifierSettings, limit: int = MAX_PER_RUN):
    """Unsent listings that clear every configured bar, best discount first."""
    qs = (
    ...
    # and ordering by ``-discount_pct`` puts whatever slipped through straight
    # into the first message the user ever receives.
    ...
    # Python. Filtering it in SQL would mean a JSON cast on every row for a
    # list that is already short by the time it gets here.
    out = []
...
def toman(value: int | None) -> str:
    ...
    every alert understate the price by a factor of ten — a 2.2B car read as
    "220M toman".
    """
    value = value or 0
...
def notify_deals(*, dry_run: bool = False) -> dict:
    ...
    for row in rows:
        ...
        if send_telegram(format_message(row), cfg.telegram_chat_id):
            ...
            # next tick instead of silently swallowing the listing forever.
