...
from apps.analytics.models import DailyInventorySnapshot
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        rows = (
            ...
            .values("model_id", "variant_id", "year", "current_price", "first_seen_at")
        )

        groups: dict = defaultdict(lambda: {"prices": [], "new": 0})
        for r in rows:
            key = (r["model_id"], r["variant_id"], r["year"])
        ...
        objs = [
            DailyInventorySnapshot(
                model_id=model_id, variant_id=variant_id, year=year, date=today,
