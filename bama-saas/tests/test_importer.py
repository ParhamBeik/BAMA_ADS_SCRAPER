...
def test_backfill_repairs_legacy_rows_and_is_idempotent():
    """A row written by the OLD code path must be fully repairable from its payload.

    Simulates pre-fix state: raw_payload intact, but year_jalali NULL, mileage
    NULL for a zero-km car, canonical_path empty, quality_flags never computed.
    """
    ...
    payload = make_payload("legacy01", 15_000_000_000)
    payload["detail"]["year"] = "2025"          # Gregorian-calendar brand
    payload["detail"]["mileage"] = "صفر کیلومتر"  # zero-km -> old code wrote NULL
    extracted = extract_ad(payload, observed)
    ad = _ing(extracted, run=run, observed_at=observed, publish_at=observed)

    # Rewind to the corrupted legacy shape.
    Ad.objects.filter(code="legacy01").update(
        year_jalali=None, year_gregorian=None, year_calendar="",
        mileage=None, canonical_path="", quality_flags=[],
    )

    call_command("backfill_normalization")
    ad.refresh_from_db()
    ...
    before = (ad.year_jalali, ad.mileage, ad.canonical_path, ad.quality_flags)
    call_command("backfill_normalization")
    ad.refresh_from_db()
    assert (ad.year_jalali, ad.mileage, ad.canonical_path, ad.quality_flags) == before


@pytest.mark.django_db
def test_backfill_purges_unrepairable_legacy_rows():
    """Legacy rows default to quality_flags=[] — which verified() treats as clean.
    A row whose payload fails a HARD rule cannot be repaired from that payload, so
    the backfill must delete it rather than leave unusable data in the table."""
    from django.core.management import call_command

    observed = datetime(2026, 7, 15, 12, 0, tzinfo=timezone.utc)
    # Written the legacy way: straight to the table, bypassing verification, so
    # the row exists exactly as pre-fix code would have left it.
    Ad.objects.create(
        code="legacy02",
        first_seen_at=observed,
        last_seen_at=observed,
        current_price=9_000_000,  # lumpsum below the price floor
        price_type="lumpsum",
        quality_flags=[],
        raw_payload=make_payload("legacy02", 9_000_000),
    )

    call_command("backfill_normalization", "--dry-run")
    assert Ad.objects.filter(code="legacy02").exists(), "--dry-run must not delete"

    call_command("backfill_normalization")
    assert not Ad.objects.filter(code="legacy02").exists()
